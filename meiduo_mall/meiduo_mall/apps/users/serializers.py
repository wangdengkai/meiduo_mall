...
class CreateUserSerializer(serializers.ModelSerializer):
    ...
    sms_code = serializers.CharField(label='短信验证码',required=True,allow_null=False,write_only=True)
    allow = serializers.CharField(label='同意协议',required=True,allow_null=False,write_only=True)
    ...
    class Meta:
        ...
        extra_kwargs={
            'username':{
                ...
                'max_length':20,
                'error_messages':{
                    'min_length':'仅仅允许5-20个字符的用户名',
                    'max_length':'仅仅允许5-20个字符的用户名',
                }
            },
            'password':{
                'write_only':True,
